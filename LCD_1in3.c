...
void LCD_1IN3_DisplayWindowsPartial(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t* buf) {
    if (x1 <= x0 || y1 <= y0) {
        return;
    }
    int width = x1 - x0;
    ...
    LCD_1IN3_SetWindows(x0, y0, x1, y1);
    DEV_Digital_Write(LCD_DC_PIN, 1);
    DEV_Digital_Write(LCD_CS_PIN, 0);
    for (int dy = 0; dy < height; dy++) {
        ...
        for (int dx = 0; dx < width; dx++) {
            uint16_t data = buf[dy * width + dx];
            temp[dx] = ((data & 0xff) << 8) | (data >> 8);
        }
        DEV_SPI_Write_nByte((uint8_t*)temp, width * 2);
    }
    DEV_Digital_Write(LCD_CS_PIN, 1);
}
